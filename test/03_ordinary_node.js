...
describe('Check log of ordinary node to find block sync issues', () => {
    it(`file ${check_fname} should be empty`, async () => {
        if (!fs.existsSync(check_fname)) {
            console.log(`***** Skip: ${path.basename(check_fname)} does not exist (watchOrdinaryNode.js not running?)`);
            return;
        }
    ...
    it(`file ${blocks_fname} should not be empty`, async () => {
        if (!fs.existsSync(blocks_fname)) {
            console.log(`***** Skip: ${path.basename(blocks_fname)} does not exist (watchOrdinaryNode.js not running?)`);
            return;
        }

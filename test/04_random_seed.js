...
describe('Check log of random seeds to find incorrect seed values', () => {
    it(`file ${seed_fname} should be empty`, async () => {
        if (!fs.existsSync(seed_fname)) {
            console.log(`***** Skip: ${path.basename(seed_fname)} does not exist (watchRandomSeed.js not running?)`);
            return;
        }
    ...
    it(`file ${seed_debug_fname} should not be empty`, async () => {
        if (!fs.existsSync(seed_debug_fname)) {
            console.log(`***** Skip: ${path.basename(seed_debug_fname)} does not exist (watchRandomSeed.js not running?)`);
            return;
        }
